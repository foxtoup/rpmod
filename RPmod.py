        ...
        try:
            ...
            if status is not False:
                if message.sender_id == me.id:
                    ...
                    if message.text.lower() == "шлеп":
                       await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> шлепнул(-а) <a href=tg://user?id={user.id}>{user.first_name}</a>") 
                    if message.text.lower() == "сделать узбеком":
                       await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> сделал(-а) узбеком <a href=tg://user?id={user.id}>{user.first_name}</a>") 
                    if message.text.lower() == "оторвать":
                       await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> оторвал(-а) член  <a href=tg://user?id={user.id}>{user.first_name}</a>") 
                    if message.text.lower() == "овца":
                       await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> ПА ПА ПА ПА <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "уложить":
